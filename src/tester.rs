...
pub fn run_tests(home_override: Option<String>) -> Result<()> {
    println!("{}", "Claude Code Deployment Diagnostic Suite".cyan().bold());
    println!("===============================================");
    ...
    let mcp_cfg_file = home.join(".config").join("claude-code").join("claude_desktop_config.json");
        ...
        if let Ok(content) = fs::read_to_string(&mcp_cfg_file) {
            if let Ok(v) = serde_json::from_str::<Value>(&content) {
                if let Some(mcp_servers) = v.get("mcpServers").and_then(|s| s.as_object()) {
                    println!("{} ({} MCP servers configured)", "✓ Found".green(), mcp_servers.len());
                } else {
